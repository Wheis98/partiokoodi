...
ohje_label = tk.Label(left_frame, text=texts_fi["instructions"], font=("Helvetica", 16, "bold"))
ohje_label.pack(pady=(0,10))

ohje_text = tk.Text(left_frame, height=20, width=35, wrap="word")
ohje_text.pack()
...
kieli_btn = tk.Button(left_frame, text="English", command=switch_language)
...
for i in range(len(room_names)):
    ...
    title = tk.Label(grid_frame, text=room_names[i], font=("Helvetica", 14, "bold"))
    title.grid(row=r*2, column=c, padx=5, pady=(0,2))
    ...
    slot_label.grid(row=r*2+1, column=c, padx=5, pady=(0,10))
