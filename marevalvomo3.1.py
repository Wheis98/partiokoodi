# admin näkym'ästä pystyy selvittämään :
# kauanko vartio on ollut rastilla ja poistaman vartion rastilta
# Lisää ja poista rastit 
# Laakson väkimäärä
# Picopi rfid lukia rastin päätyyn
#
#
#
#
...
def get_vartio_checkpoints(tag_uid: int):
    c.execute("SELECT DISTINCT r.nimi_fi FROM loki as l INNER JOIN rastit as r on l.rasti_id = r.id WHERE l.tag_idx = (?) ",
            (tag_uid,))
    #c.fetchall() palauttaa tuplen rastien nimistä, joista palautetaan lista rastin numeroita
    return [i[0].split(" ")[0] for i in c.fetchall()]
...
def choose_rasti_window(tag_uid: int, current_idx: Optional[int]):
    ...
    kaydyt = get_vartio_checkpoints(tag_uid)
    print(kaydyt)
    ...
    if current_idx is not None:
        msg = (f"{tag_str} on nyt rastilla {get_room_name(current_idx)} \n {tag_str} on käynyt rasteilla {kaydyt}" if kieli == "FI" else f"{tag_str} is now at {get_room_name(current_idx)} \n"+
        ...
        msg = f"{tag_str} luettu. Valitse rasti: \n {tag_str} on käynyt rasteilla {kaydyt} " if kieli == "FI" else f"{tag_str} read. Choose checkpoint:"

    vrt = f"Vartio: {vartio_nimi}, koko: {vartio_koko}." if kieli == "FI" else f"Group: {vartio_nimi}, size: {vartio_koko}."
    tk.Label(win, text=msg, font=("Helvetica", 13)).pack(pady=10)
    tk.Label(win, text=vrt, font=("Helvetica", 13)).pack(pady=10)
...
# Käyttö esimerkissämme (esim. choose_rasti_window-funktiossa):
    ...
    for i, nimi in enumerate(room_names if kieli == "FI" else room_names_en):
        b = tk.Button(grid, text=f"{nimi} ({TOTAL_SLOTS[i] - occupied_slots[i]})", width=20, command=lambda i=i: assign(i))
        b.grid(row=i // 3, column=i % 3, padx=7, pady=5)
